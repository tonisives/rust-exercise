...
pub fn frequency(input: &[&str], worker_count: usize) -> HashMap<char, usize> {
    ...
    // each processes a slice from input
    // returns char count map for the slice
    // in the end, merge the slices
    let handles: Vec<_> = chunks.map(|chunk| spawn_counter(chunk).join()).collect();
    handles.iter().for_each(|handle| match handle {
        Ok(handle_ok) => handle_ok
            .iter()
            .for_each(|(char, size)| *res.entry(*char).or_insert(0) += size),
        Err(_) => (),
    });
...
fn spawn_counter(input: &[&str]) -> JoinHandle<HashMap<char, usize>> {
    let copy: Vec<String> = input.iter().map(|c| c.to_string()).collect();

    spawn(move || {
        let mut res: HashMap<char, usize> = HashMap::new();

        copy.iter().for_each(|str| {
